...
// Пароль администратора ADMIN_PASSKEY должен быть установлен как секрет в Koyeb.
// Приложение завершит работу, если этот секрет не будет найден.
const ADMIN_PASSKEY = process.env.ADMIN_PASSKEY;

if (!ADMIN_PASSKEY) {
    console.error('КРИТИЧЕСКАЯ ОШИБКА: Секрет ADMIN_PASSKEY не установлен в переменных окружения!');
    console.error('Убедитесь, что секрет ADMIN_PASSKEY задан в настройках вашего сервиса Koyeb.');
    process.exit(1); // Завершить работу, если пароль администратора не установлен
}
...
// Сейчас они доступны любому, кто знает URL и пароль (если он передается в теле).
// Рекомендуется использовать JWT-аутентификацию для администратора.

app.post('/api/admin/setpro', (req, res) => {
    const { username, isPro, adminSecret } = req.body;
    if (adminSecret !== ADMIN_PASSKEY) return res.status(403).json({ error: 'Нет доступа (setpro - неверный пароль администратора)' });
...
app.get('/api/admin/pending', (req, res) => {
    // TODO: Этот GET эндпоинт должен быть защищен (например, JWT админа в заголовке).
    // Текущая реализация не безопасна для производственного использования без дополнительной защиты.
    // Для примера, можно было бы требовать adminSecret в query параметрах, но это плохая практика для GET.
    // if (req.query.admin_secret !== ADMIN_PASSKEY) return res.status(403).json({ error: 'Нет доступа' });
    res.json(db.pending.map(u => ({ username: u.username, displayName: u.displayName })));
});

app.get('/api/admin/users', (req, res) => {
    // TODO: Этот GET эндпоинт должен быть защищен.
    res.json(db.users.map(u => ({ username: u.username, displayName: u.displayName, isPro: !!u.isPro })));
});

app.post('/api/admin/approve', (req, res) => {
    const { username, accept, adminSecret } = req.body;
    if (adminSecret !== ADMIN_PASSKEY) return res.status(403).json({ error: 'Нет доступа (approve - неверный пароль администратора)' });
...
const defaultConfig = { monitorUrl: LLAMA_SERVER_URL + '/data' };
let config;
try {
...
app.listen(PORT, () => {
    ...
    // Лог о ADMIN_PASSKEY теперь не нужен, так как приложение завершится, если он не установлен.
    // Вместо этого, можно просто подтвердить, что он используется (если он не был undefined и не вызвал process.exit).
    console.log('Пароль администратора (ADMIN_PASSKEY) инициализирован из переменных окружения.');
